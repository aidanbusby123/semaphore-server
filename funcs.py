...
def parse_message(buf, type):
    ...
    if len(buf) >= 72:
        if type == MESSAGE:
            ...
            message_tuple[4] = buf[72:72+int.from_bytes(message_tuple[2], sys.byteorder)]
        elif type == CON:
            message_tuple[0] = buf[0:32]
            message_tuple[1] = buf[32:36]
            message_tuple[2] = buf[36:40]
            content_len = int.from_bytes(message_tuple[2], sys.byteorder)
            message_tuple[3] = buf[40:40+content_len]
            message_tuple[4] = buf[40+content_len:40+content_len+4]
            signature_len = int.from_bytes(message_tuple[4], sys.byteorder)
